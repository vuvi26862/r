window.addEventListener("DOMContentLoaded", () => {
  ...
  function handleResponsiveLayout() {
    ...
    if (isMobile()) {
      ...
      const touchElements = document.querySelectorAll(
        ".major-card, .bento-item, .event-card, .alumni-card"
      );
  ...
  function handleScrollAnimation() {
    ...
    scrollElements.forEach((el) => {
      ...
      if (
        elementTop < windowHeight - triggerOffset &&
        elementBottom > triggerOffset
      ) {
  ...
  function showMascotEgg() {
    ...
    document
      ...
      .forEach((f) => f.classList.add("hide-on-easter-egg"));
  }
  ...
  function hideMascotEgg() {
    ...
    document
      .querySelectorAll("form")
      .forEach((f) => f.classList.remove("hide-on-easter-egg"));
  }

  if (mascotClose) mascotClose.onclick = hideMascotEgg;
  if (mascotMusic)
    mascotMusic.onclick = function () {
  ...
  if (isMobile()) {
    ...
    const animatedElements = document.querySelectorAll(
      ".major-card, .bento-item, .event-card"
    );
  ...
  gate.addEventListener("click", () => {
    ...
    if (countclick >= 8) {
      ...
      window.location.href =
        "https://www.youtube.com/watch?v=dQw4w9WgXcQ&list=RDdQw4w9WgXcQ&start_radio=1"; // ← thay bằng link thật
    }
  });
});
